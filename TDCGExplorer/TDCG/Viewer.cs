...
namespace TDCG
{
    ...
    public class Viewer : IDisposable
    {
        ...
        public void AddFigureFromPNGFile(string source_file, bool append)
        {
            ...
            AddFigureFromPNGStream(stream, append);
        }
        ...
        /// <param name="source_file">PNGFile のパス</param>
        /// <param name="append">FigureListを消去せずに追加するか</param>
        public void AddFigureFromPNGStream(Stream stream, bool append)
        {
        ...
        {
            Surface surface = device.CreateOffscreenPlainSurface(device.DisplayMode.Width, device.DisplayMode.Height, Format.A8R8G8B8, Pool.SystemMemory);
            Surface backsf = device.GetBackBuffer(0, 0, BackBufferType.Mono);
            device.GetRenderTargetData(backsf, surface);
            Graphics gorg = surface.GetGraphics();　//Graphicsの取得
		    Bitmap bitmap = new Bitmap(surface.Description.Width,surface.Description.Height,gorg);
            Graphics gdst = Graphics.FromImage(bitmap);
		    IntPtr srcHDC = gorg.GetHdc();  //Hdcの取得
		    IntPtr destHDC = gdst.GetHdc();
		    BitBlt(destHDC,0,0,bitmap.Width,bitmap.Height,srcHDC,0,0,0xCC0020);　//転送
		    gdst.ReleaseHdc(destHDC);  //ここら辺はマネージドじゃないので解放せねば
		    gorg.ReleaseHdc(srcHDC);
